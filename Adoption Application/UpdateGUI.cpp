...
UpdateGUI::UpdateGUI(Service& _service, QTableWidget* _dataTable) : service(_service), dataTable(_dataTable) {

	saveButton = new QPushButton("Update");
	connect(saveButton, &QPushButton::released, this, &UpdateGUI::handleSaveButton);

	ageLabel = new QLabel("New age: ");
	nameLabel = new QLabel("Name: ");
	breedLabel = new QLabel("Breed: ");
	photoLabel = new QLabel("New photolink: ");
...
void UpdateGUI::handleSaveButton() {
	...
	try {
		service.updateDog(name, breed, age, photo);
	}
	catch (exception& e) {
		QMessageBox msgBox;
...
void UpdateGUI::updateDataTable() {
	dataTable->clear();
